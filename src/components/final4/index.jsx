...
export const QuartasFinal = () => {
    const navigate = useNavigate();
    const location = useLocation();
    const { setJogadores } = useContext(JogadorContext);

    const classificadosDasOitavas = location.state?.classificados || [];
    ...
    useEffect(() => {
        ...
        setVencedoresQuartas(listaVencedores);
    ...
    return (
        <div className={styles.int}>
            ...
            <div className={styles.main}>
                {confrontos.length > 0 ? confrontos.map((duelo, index) => (
                ...
                )) : <h1>Carregando duelos...</h1>}
            </div>

            <div className={styles.buttonDiv}>
                <button 
                    className={styles.button} 
                    onClick={() => navigate("/semifinal", { state: { classificados: vencedoresQuartas, historico: [...location.state.historico, ...historicoDestaRodada] } })}
                >
